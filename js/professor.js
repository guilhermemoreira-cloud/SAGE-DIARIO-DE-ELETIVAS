// js/professor.js - Lógica do professor (VERSÃO CORRIGIDA COM SINCRONIZAÇÃO EM TEMPO REAL)
...
// ========== FUNÇÃO PARA OBTER TEMPO DA ELETIVA ==========
...
// ========== FUNÇÃO PARA RECARREGAR AS ELETIVAS DO PROFESSOR ==========
function recarregarEletivasProfessor() {
  console.log("🔄 Recarregando eletivas do professor...");
    ...
    try {
      ...
      if (window.FirebaseSync && typeof window.FirebaseSync.carregarColecoesGestor === 'function') {
        ...
        if (carregou) {
          ...
          recarregarEletivasProfessor();
        }
      }

      // 🔥 CONFIGURAR LISTENER EM TEMPO REAL PARA TODAS AS COLEÇÕES RELEVANTES
      if (typeof window.FirebaseSync.escutarColecoesGestor === 'function') {
        window.FirebaseSync.escutarColecoesGestor(function (colecao) {
          console.log(`🔄 Atualização em tempo real recebida: ${colecao}`);
          
          // Recarregar dados conforme a coleção que mudou
          if (colecao === 'eletivas' || colecao === 'professores' || colecao === 'matriculas') {
            console.log("📚 Recarregando eletivas do professor...");
            recarregarEletivasProfessor();
          }
          
          if (colecao === 'registros' || colecao === 'notas') {
            console.log("📋 Recarregando registros realizados...");
            if (document.getElementById("tab-registros")?.classList.contains("active")) {
              carregarRegistrosRealizados();
            }
            // Também atualizar os cards do dashboard
            carregarEletivasProfessor();
          }
          
          if (colecao === 'liberacao_notas') {
            console.log("🔓 Liberação de notas atualizada...");
            carregarEletivasProfessor();
          }
        });
      }
...
function carregarEletivasProfessor() {
  ...
  // Filtrar eletivas do professor atual
  const eletivas = state.eletivas.filter((e) => normalizarIdParaComparacao(e.professorId) === normalizarIdParaComparacao(professorAtual.id));
  ...
  Object.keys(eletivasPorDia).forEach((dia) => {
    ...
    eletivasDoDia.forEach((eletiva) => {
      const matriculas = state.matriculas.filter((m) => normalizarIdParaComparacao(m.eletivaId) === normalizarIdParaComparacao(eletiva.id)).length;

      const registrosEletiva =
        state.registros?.filter((r) => normalizarIdParaComparacao(r.eletivaId) === normalizarIdParaComparacao(eletiva.id))
      ...
      const notaExistente = state.notas?.find((n) => normalizarIdParaComparacao(n.eletivaId) === normalizarIdParaComparacao(eletiva.id) && n.semestre === semestreAtual);
...
function carregarAlunosParaChamada(eletivaId) {
  ...
  const alunos = state.alunos.filter((a) => matriculas.some((m) => normalizarIdParaComparacao(m.alunoId) === normalizarIdParaComparacao(a.id)));
...
window.salvarRegistroAula = function (event) {
  ...
  const registroExistente = state.registros?.find(
    (r) => normalizarIdParaComparacao(r.eletivaId) === normalizarIdParaComparacao(eletivaSelecionadaRegistro) && 
  ...
  const registro = {
    ...
    eletivaId: String(eletivaSelecionadaRegistro),
    ...
    professorId: String(professorAtual.id),
...
window.abrirModalNotas = function (eletivaId, semestre = "1/2026") {
  ...
  const registroExistente = state.notas?.find((n) => normalizarIdParaComparacao(n.eletivaId) === normalizarIdParaComparacao(eletivaId) && n.semestre === semestre);
...
window.salvarNotas = async function () {
  ...
  inputs.forEach((item) => {
    ...
    if (!isNaN(valor) && valor >= 0 && valor <= 10) {
      const aluno = state.alunos.find((a) => normalizarIdParaComparacao(a.id) === normalizarIdParaComparacao(alunoId));
      if (aluno) {
        notas.push({
          alunoId: alunoId,
          nome: aluno.nome,
  ...
  const registroNotas = {
    ...
    eletivaId: String(registroNotasEmEdicao.eletivaId),
  ...
  const index = state.notas.findIndex(
    (n) => normalizarIdParaComparacao(n.eletivaId) === normalizarIdParaComparacao(registroNotas.eletivaId) && 
...
window.carregarRegistrosRealizados = async function () {
  ...
  if (!registros || registros.length === 0) {
    registros = (window.state?.registros || []).filter((r) => normalizarIdParaComparacao(r.professorId) === normalizarIdParaComparacao(professorAtual.id));
  }

  if (filtroEletiva) {
    registros = registros.filter((r) => normalizarIdParaComparacao(r.eletivaId) === normalizarIdParaComparacao(filtroEletiva));
  }
  ...
  const eletivasProfessor = state.eletivas?.filter((e) => normalizarIdParaComparacao(e.professorId) === normalizarIdParaComparacao(professorAtual?.id)) || [];
...
window.salvarEdicaoRegistro = async function (event) {
  ...
  const registroConflitante = state.registros?.find(
    (r) =>
      ...
      normalizarIdParaComparacao(r.eletivaId) === normalizarIdParaComparacao(registroEmEdicao.eletivaId) &&
      normalizarDataParaSalvar(r.data) === dataNormalizada
  );
...
async function excluirRegistro(registroId) {
  ...
  try {
    const idString = String(registroId);

    const index = state.registros.findIndex((r) => String(r.id) === idString);
    if (index !== -1) {
    ...
    document.querySelectorAll(".eletiva-card.registros-card").forEach((card) => {
      ...
      if (eletivaId) {
        const count = state.registros.filter((r) => String(r.eletivaId) === eletivaId).length;
...
window.imprimirRegistrosPorData = async function (eletivaId, dataISO) {
  ...
  try {
    ...
    if (!registros || registros.length === 0) {
      registros = (window.state?.registros || []).filter((r) => normalizarIdParaComparacao(r.eletivaId) === normalizarIdParaComparacao(eletivaId));
    }
...
window.imprimirPDFNotas = async function (eletivaId, semestre = "1/2026") {
  ...
  try {
    ...
    const registroNotas = state.notas?.find((n) => normalizarIdParaComparacao(n.eletivaId) === normalizarIdParaComparacao(eletivaId) && n.semestre === semestre);
