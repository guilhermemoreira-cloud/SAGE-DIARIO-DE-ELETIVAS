...
let configTemposPadrao = {
  T1: { diaSemana: "SEGUNDA-FEIRA", series: ["1ª", "2ª", "3ª"] },
  T2: { diaSemana: "QUINTA-FEIRA", series: ["1ª", "3ª"] },
  T3: { diaSemana: "TERÇA-FEIRA", series: ["1ª"] },
  T4: { diaSemana: "SEXTA-FEIRA", series: ["1ª"] },
  T5: { diaSemana: "QUARTA-FEIRA", series: ["1ª"] },
};
...
let liberacaoNotasPadrao = {
  ...
  periodo: {
    ...
    fim: new Date(new Date().setDate(new Date().getDate() + 10)).toISOString().split("T")[0],
  },
  eletivasLiberadas: [],
};
...
const diasSemanaOpcoes = ["segunda", "terca", "quarta", "quinta", "sexta"];
...
window.exportarJSONCompleto = function () {
  try {
    const dadosExport = {
      ...
      conflitos: state.conflitos || { resolvidosAutomaticamente: [], pendentes: [] },
    };
...
function carregarProfessores() {
  ...
  document.getElementById("contadorProfessores").textContent = `(${professores.length} professores encontrados)`;
  ...
  professores.forEach((professor) => {
    ...
    if (eletivas.length > 0) {
      ...
      eletivas.slice(0, 3).forEach((e) => {
        ...
        const tempo = getTempoFromHorario(e.horario) || "N/A";
        eletivasHTML += `
          <div class="eletiva-item">
            <span class="eletiva-nome">${e.nome} (${e.codigo})</span>
            <span class="eletiva-detalhes">${e.horario?.diaSemana} ${tempo} - ${matriculas.length} alunos</span>
          </div>
        `;
      });
...
window.salvarProfessor = async function () {
  ...
  try {
    if (professorEmEdicao) {
      ...
      if (index !== -1) {
        state.professores[index] = { ...state.professores[index], nome: nome, email: email };
        salvarEstado();
      ...
      const novoProfessor = { id: novoId, nome: nome, email: email, perfil: "PROFESSOR" };
...
async function removerProfessor(professorId) {
  ...
  try {
    ...
    if (window.FirebaseSync) {
      await window.FirebaseSync.salvarDadosFirebase("professores", null, professorId);
    }
...
function carregarLocais() {
  if (state.locais && state.locais.length > 0) {
    locais = state.locais;
  } else {
    locais = [
...
function carregarEletivas() {
  ...
  document.getElementById("contadorEletivas").textContent = `(${eletivas.length} eletivas encontradas)`;
...
// ========== FUNÇÃO SALVAR ELETIVA CORRIGIDA ==========
...
window.salvarCategoriaEletiva = async function () {
  ...
  if (eletivaEmEdicao.tipo === "MISTA" && novaCategoria === "FIXA") {
    const matriculas = state.matriculas?.filter((m) => m.eletivaId === eletivaEmEdicao.id) || [];
    const alunos = state.alunos?.filter((a) => matriculas.some((m) => m.alunoId === a.id)) || [];

    if (alunos.length > 0) {
      const primeiraTurma = alunos[0].turmaOrigem;
      const todosMesmaTurma = alunos.every((a) => a.turmaOrigem === primeiraTurma);
      ...
      if (!todosMesmaTurma) {
        ...
        confirmBody.innerHTML = `
          ...
          <ul style="margin-left: 1.5rem; margin-top: 0.5rem;">
            ${alunos.slice(0, 5).map((a) => `<li>${a.nome} - ${a.turmaOrigem}</li>`).join("")}
...
async function executarAlteracaoCategoria(eletivaId, novaCategoria) {
  ...
  try {
    ...
    if (index !== -1) {
      ...
      state.eletivas[index] = { ...state.eletivas[index], tipo: novaCategoria };
...
async function removerEletiva(eletivaId) {
  ...
  try {
    ...
    if (window.FirebaseSync) {
      await window.FirebaseSync.salvarDadosFirebase("eletivas", null, eletivaId);
    }
...
function estudanteEstaNaEletiva(estudanteId, eletivaId) {
  return state.matriculas?.some((m) => m.alunoId === estudanteId && m.eletivaId === parseInt(eletivaId));
}
...
function getEletivasEstudante(estudanteId) {
  ...
  matriculas.forEach((m) => {
    ...
    if (eletiva) {
      const professor = state.professores?.find((p) => p.id === eletiva.professorId)?.nome || "Não atribuído";
      const tempo = getTempoFromHorario(eletiva.horario) || "N/A";
      eletivas.push({ ...eletiva, professorNome: professor, tempo: tempo });
    }
  });
...
window.filtrarEstudantes = function () {
  ...
  if (busca) {
    estudantes = estudantes.filter((e) => e.nome?.toLowerCase().includes(busca) || e.codigoSige?.includes(busca));
  }
...
function carregarEletivasCheckbox(estudanteId = null) {
  ...
  const eletivasPorTempo = { T1: [], T2: [], T3: [], T4: [], T5: [], OUTROS: [] };
  ...
  const nomesTempo = {
    T1: "TEMPO 1 - SEGUNDA-FEIRA",
    T2: "TEMPO 2 - QUINTA-FEIRA",
    T3: "TEMPO 3 - TERÇA-FEIRA",
    T4: "TEMPO 4 - SEXTA-FEIRA",
    T5: "TEMPO 5 - QUARTA-FEIRA",
...
window.salvarEstudante = async function () {
  ...
  try {
    if (estudanteEmEdicao) {
      ...
      if (index !== -1) {
        state.alunos[index] = { ...state.alunos[index], nome: nome, turmaOrigem: turma };

        if (window.FirebaseSync) {
          await window.FirebaseSync.salvarDadosFirebase("alunos", state.alunos[index], state.alunos[index].id);
        }

        state.matriculas = state.matriculas.filter((m) => m.alunoId !== estudanteEmEdicao.id);
...
window.confirmarRemoverEstudante = function (estudanteId) {
  ...
  const matriculas = state.matriculas?.filter((m) => m.alunoId === estudanteId) || [];
  const eletivas = matriculas.map((m) => {
...
async function removerEstudante(estudanteId) {
  ...
  try {
    ...
    if (window.FirebaseSync) {
      await window.FirebaseSync.salvarDadosFirebase("alunos", null, estudanteId);
    }
...
function atualizarStatusDados() {
  ...
  document.getElementById("ultimaImportacao").textContent = metadata.ultimaImportacao ? new Date(metadata.ultimaImportacao).toLocaleString() : "Nunca";
  ...
  document.getElementById("totalFixas").textContent = dados.eletivasFixas?.length || state.eletivas?.filter((e) => e.tipo === "FIXA").length || 0;
  document.getElementById("totalMistas").textContent = dados.eletivasMistas?.length || state.eletivas?.filter((e) => e.tipo === "MISTA").length || 0;
  ...
  if (conflitos.resolvidosAutomaticamente?.length > 0 || conflitos.pendentes?.length > 0) {
    ...
    if (conflitos.resolvidosAutomaticamente?.length > 0) {
      choquesResolvidos.innerHTML = `
        ...
        <ul style="margin-left: 1.5rem;">
          ${conflitos.resolvidosAutomaticamente.slice(0, 5).map((c) => `<li>${c.motivo || `Aluno removido de ${c.detalhes?.removidos?.length || 1} fixa(s) (prioridade mista) - Tempo ${c.tempo}`}</li>`).join("")}
...
window.importarJSONSage = async function () {
  ...
  try {
    ...
    const eletivasFixas = (dadosNovos.dados.eletivasFixas || []).map((e) => ({ ...e, tipo: "FIXA", id: e.codigo ? e.codigo : `FIXA_${Math.random()}` }));
    const eletivasMistas = (dadosNovos.dados.eletivasMistas || []).map((e) => ({ ...e, tipo: "MISTA", id: e.codigo ? e.codigo : `MISTA_${Math.random()}` }));
    ...
    if (window.FirebaseSync) {
      setTimeout(() => {
        window.FirebaseSync.salvarDadosFirebase("dados_completos", dadosMesclados);
      }, 100);
    }
    ...
    if (resultadoChoques.conflitosPendentes.length > 0) {
      const fixas = resultadoChoques.conflitosPendentes.filter((c) => c.tipo === "fixa").length;
      const mistas = resultadoChoques.conflitosPendentes.filter((c) => c.tipo === "mista").length;

      showToast(`⚠️ Importação concluída com ${resultadoChoques.conflitosPendentes.length} conflitos pendentes.\n${resultadoChoques.remocoesAutomaticas.length} conflitos resolvidos automaticamente (MISTA prioritária).`, "warning");
    } else {
    ...
    Object.entries(alunoEletivas).forEach(([alunoId, eletivasDoAluno]) => {
      ...
      if (mistas.length > 0 && fixas.length > 0) {
        ...
        remocoesAutomaticas.push({
          ...
          detalhes: {
            mantidas: mistas.map((m) => ({ nome: m.nome, eletivaId: m.eletivaId })),
            removidos: fixasRemover.map((f) => ({ nome: f.nome, eletivaId: f.eletivaId })),
          },
        });
        ...
        conflitosPendentes.push({
          ...
          tempo,
          tipo: "fixa",
          eletivas: fixas.map((f) => ({ nome: f.nome, eletivaId: f.eletivaId })),
          gravidade: "alta",
        ...
        conflitosPendentes.push({
          ...
          tempo,
          tipo: "mista",
          eletivas: mistas.map((m) => ({ nome: m.nome, eletivaId: m.eletivaId })),
          gravidade: "media",
...
window.abrirModalEditarTempos = function () {
  ...
  ["T1", "T2", "T3", "T4", "T5"].forEach((tempo) => {
    ...
    html += `
      <div class="tempo-edit-item" style="display: flex; gap: 1rem; align-items: flex-start; padding: 1rem; margin-bottom: 1rem; background: var(--bg-light); border-radius: 8px; border-left: 4px solid var(--primary);">
  ...
  try {
    ...
    ["T1", "T2", "T3", "T4", "T5"].forEach((tempo) => {
      ...
      ["1ª", "2ª", "3ª"].forEach((serie) => {
        const checkbox = document.getElementById(`tempo_${tempo}_serie_${serie}`);
    ...
    if (window.FirebaseSync) {
      await window.FirebaseSync.salvarDadosFirebase("config_tempos", novaConfig);
    }
...
window.abrirModalEditarLiberacao = function () {
  ...
  const liberacao = state.liberacaoNotas || {
    semestre: "1/2026",
    periodo: { inicio: formatarDataInput(inicioPadrao), fim: formatarDataInput(fimPadrao) },
    eletivasLiberadas: [],
  };
  ...
  if (eletivas.length === 0) {
    container.innerHTML = '<p class="empty-state">Nenhuma eletiva cadastrada</p>';
...
window.salvarLiberacaoNotas = async function () {
  ...
  try {
    ...
    document.querySelectorAll(".eletiva-liberacao-checkbox:checked").forEach((cb) => {
    ...
    if (window.FirebaseSync) {
      await window.FirebaseSync.salvarDadosFirebase("liberacao_notas", novaConfig);
    }
    ...
    if (window.FirebaseSync) {
      if (state.professores) {
        for (const prof of state.professores) {
          await window.FirebaseSync.salvarDadosFirebase("professores", prof, prof.id);
        }
      }
      if (state.alunos) {
        for (const aluno of state.alunos) {
          await window.FirebaseSync.salvarDadosFirebase("alunos", aluno, aluno.id);
        }
      }
      if (state.eletivas) {
        for (const eletiva of state.eletivas) {
          await window.FirebaseSync.salvarDadosFirebase("eletivas", eletiva, eletiva.id);
        }
      }
      if (state.matriculas) {
        for (const mat of state.matriculas) {
          await window.FirebaseSync.salvarDadosFirebase("matriculas", mat, mat.id);
        }
      }
      if (state.registros) {
        for (const reg of state.registros) {
          await window.FirebaseSync.salvarDadosFirebase("registros", reg, reg.id);
        }
      }
      if (state.notas) {
      ...
      if (state.configTempos) {
        await window.FirebaseSync.salvarDadosFirebase("config_tempos", state.configTempos);
      }
      if (state.liberacaoNotas) {
        await window.FirebaseSync.salvarDadosFirebase("liberacao_notas", state.liberacaoNotas);
      }
    }
...
window.abrirModalConflitos = function () {
  ...
  conflitos.forEach((conflito, index) => {
    ...
    html += `
      <div class="conflito-item ${tipoClasse}" style="margin-bottom: 1rem; padding: 1rem; background: var(--bg-white); border-radius: 8px; border-left: 4px solid ${corBorda}; box-shadow: var(--shadow);">
        ...
        <ul style="margin: 0 0 0.5rem 1.5rem;">
          ${(conflito.eletivas || []).map((e) => {
            const eletiva = state.eletivas?.find((el) => el.id === e.eletivaId || el.codigo === e.eletivaId);
            const prof = state.professores?.find((p) => p.id === eletiva?.professorId)?.nome || "N/A";
        ...
        <div style="margin-top: 0.5rem; padding: 0.5rem; background: ${corBorda}20; border-radius: 4px;">
          <p style="margin: 0; font-size: 0.9rem;">
            ...
            ${conflito.tipo === "fixa" ? "Eletivas FIXAS exigem que o aluno esteja em apenas UMA por tempo. Remova uma das matrículas." : "Eletivas MISTAS também permitem apenas UMA por tempo. Remova uma das matrículas."}
          </p>
        </div>
      </div>
    `;
